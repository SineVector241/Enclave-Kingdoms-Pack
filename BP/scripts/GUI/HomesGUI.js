...
const chooseHome = new ActionFormData().title("§2Choose Home");
      ...
      switch (ev.selection) {
        case 0: //Warp Home
          if (player.hasTag("inCombat")) {
            ...
            const lastWarped =
              player.getDynamicProperty("enclave:lastWarpped") || 0;
            ...
            if (lastWarped + Cooldown < rightNow) {
              ...
              player.sendMessage(
                `§cYou must wait ${Math.floor(
                  (lastWarped + Cooldown - rightNow) / 1000
                )} seconds before warping again.`
              );
            }
          }
          break;
        ...
        case 1: //Set Home
          ...
          confirm.show(player).then((resp) => {
            ...
            player.sendMessage(
              NumberOfHomes > 1 ? `§aHome ${Home} Set.` : `§aHome Point Set.`
            );
          });
          break;
        ...
        case 2: { //Home Coordinates
          ...
          player.sendMessage(
            NumberOfHomes > 1
              ? `§aYour Home ${Home} is at §d${Math.round(
                  home[1]
                )}, §e${Math.round(home[2])}, §b${Math.round(
                  home[3]
                )} §ain ${home[0].replace("minecraft:", "")}`
              : `§aYour home is at §d${Math.round(home[1])}, §e${Math.round(
                  home[2]
                )}, §b${Math.round(home[3])} §ain ${home[0].replace(
                  "minecraft:",
                  ""
                )}`
          );
